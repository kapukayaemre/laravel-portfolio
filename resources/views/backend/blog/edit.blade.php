    ...
    <div class="card mb-5 mb-xl-10">
        ...
        <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse"
            ...
            <div class="card-title m-0">
                <h3 class="fw-bolder m-0">Blog Bilgilerini Güncelleme Bölümü</h3>
            </div>
            <!--end::Card title-->
        </div>
        <!--begin::Card header-->
        ...
        <div id="kt_account_profile_details" class="collapse show">
            <!--begin::Form-->
            <form action="{{ route("admin.blog.update", $blog->id) }}" method="POST" class="form fv-plugins-bootstrap5 fv-plugins-framework" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <!--begin::Card body-->
                        ...
                        <div class="col-lg-8 fv-row fv-plugins-icon-container">
                            <select name="category_id" aria-label="Select a Country" data-control="select2" data-placeholder="Kategori Seçiniz" class="form-select form-select-solid form-select-lg fw-bold select2-hidden-accessible" data-select2-id="select2-data-10-qusw" tabindex="-1" aria-hidden="true">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old("category_id") == $category->id ? "selected" : "" }} {{ isset($blog) ? ($blog->category_id === $category->id ? "selected" : "") : "" }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                            <div class="fv-plugins-message-container invalid-feedback"></div>
                        </div>
